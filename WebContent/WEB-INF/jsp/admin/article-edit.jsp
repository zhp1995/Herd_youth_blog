...
<article class="page-container">
	<form class="form form-horizontal" id="form-article-edit">
		<div class="row cl">
	    	<input type="hidden"  value="${article.article_id}" name="articleId">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="introduction"  id="introduction" class="textarea"  placeholder="说点什么...最少输入10个字符" >${article.introduction}</textarea>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">类别：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${article.catelogy}" placeholder="" id="catelogy" name="catelogy">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">浏览量：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${article.readSum }" placeholder="" id="readSum" name="readSum">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">点赞量：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${article.goodSum}" placeholder="" id="goodSum" name="goodSum">
			</div>
		</div>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			     <img width="100" height="100" src="${path}/getPic.shtml?picName=${article.pic}" id="allImgUrl" />
			     <!-- 实际传给后台表单的标签 -->
			     <input type="hidden" name="pic" value="${article.pic}" id="path"/>
			...
			<div class="formControls col-xs-8 col-sm-9"> 
				<textarea id="editor" name="content" style="width:100%;height:600px;" >${article.content}</textarea>
			</div>
		</div>
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="article_edit();" class="btn btn-secondary radius" type="button"><i class="Hui-iconfont">&#xe632;</i> 提交</button>
...
<script type="text/javascript" src="${path}/js/static/h-ui.admin/js/H-ui.admin.js"></script> <!--/_footer /作为公共模版分离出去-->
...
function uploadPic(){
	...
	//jquery.form使用方式
   $("#form-article-edit").ajaxSubmit(options);
}
...
function article_edit(){
	...
	if (msg != ""){
		...
		var dd = $("#form-article-edit").serialize();
		$.ajax("${path}/article/admin/editArticle.shtml",{
			type:"post",
			data:dd,
			dataType:"json",
			async:false,
